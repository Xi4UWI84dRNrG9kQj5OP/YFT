...
///64 bit Impl with no fixed group size and with child pointer
pub struct YFT {
...
impl YFT {
    ...
    pub fn new(elements: Vec<DataType>, args: &Args, log: &mut Log) -> YFT {
        ...
        for (pos, value) in elements.iter().enumerate() {
            ...
            //set successors
            if lss_top[lss_top_pos] == DataType::max_value() && !is_left_child(YFT::lss_top_position(value, last_level_len + 1)) {
        ...
        for (element_array_index, value) in elements.iter().enumerate() {
            ...
            if Some(x_leaf_position) == predecessor_x_leaf {
                ...
                let new_node = TreeLeaf { first_element: element_array_index };
            ...
            for i in 1..levels {
                ...
                lss_branch[i - 1].entry(path).and_modify(|parent: &mut TreeBranch| {
                    ...
                    if insert {
                        ...
                        parent.descending = element_array_index;
                    }
                }).or_insert_with(|| {
                    //case no parent -> create one
                    debug_assert!(insert);
                    TreeBranch { children: if is_left_child { Children::LEFT } else { Children::RIGHT }, descending: element_array_index }
                });
                child = path;
            }
                ...
                match self.lss_branch[search_range.0 - 1].get(&calc_path(query, search_range.0, self.start_level)) {
                    Some(branch) => {
                        if !branch.has_right_child() {
                            ...
                            return if branch.descending == 0 { None } else { self.element_from_array(query, branch.descending - 1) };
                        }
                    }
                    None => {
                        panic!("This can't happen, cause it was checked at beginning of this method, that there is a predecessor");
    ...
    fn predecessor_from_array(&self, query: DataType, index: DataType) -> Option<DataType> {
        let mut index = index;
        while index < self.elements.len() && self.elements[index] < query {
            index += 1;
        }
        return if index == 0 { None } else { self.element_from_array(query, index - 1) };
    }
    ...
    pub fn predecessor_with_stats(&self, position: DataType) -> (Option<DataType>, u32, u32) {
        let mut count = 0;
        unsafe {
            if position < *self.elements.get_unchecked(43) {
                return (None, 0, count);
            }
...
fn calc_path(position: DataType, lss_level: usize, start_level: usize) -> DataType {
    position >> (lss_level + start_level)
}
