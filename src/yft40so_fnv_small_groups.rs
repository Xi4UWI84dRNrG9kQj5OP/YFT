...
///40 bit Impl with calculated group size and without child pointer and with binary search below leafs
pub struct YFT {
...
impl YFT {
    ...
    pub fn new(elements: Vec<DataType>, args: &Args, log: &mut Log) -> YFT {
        ...
        let group_size = YFT::calc_min_group_size(&elements, start_level);
    ...
    pub fn print_stats(&self, log: &Log) {
        ...
        log.print_result(format!("level=-1\tnodes={}\telements={}\tgroup_size={}", count, self.elements.len(), self.group_size));
    }
    ...
    fn calc_min_group_size(elements: &Vec<DataType>, start_level: usize) -> usize {
        let mut group_size = 1;
        let mut current_group_size = 0;
        let mut current_prefix = DataType::from(0);
        for element in elements {
            let prefix = calc_path(*element, 0, start_level);
