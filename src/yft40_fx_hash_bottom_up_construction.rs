...
impl YFT {
    ...
    pub fn new(elements: Vec<DataType>, args: &Args, log: &mut Log) -> YFT {
        ...
        for (element_array_index, value) in elements.iter().enumerate() {
            ...
            if !lss_leaf.contains_key(&leaf_position) {
                lss_leaf.insert(leaf_position, TreeLeaf { first_element: DataType::from(element_array_index) });
            }
        }
        ...
        for i in 1..levels {
            let mut branch_map = FxHashMap::default();
            for (element_array_index, value) in elements.iter().enumerate() { //TODO iter through level above instead of elements
                ...
                branch_map.entry(branch_position).and_modify(|branch_node: &mut TreeBranch| {
                    if (!branch_node.has_left_child() && is_left_child) || (!branch_node.has_right_child() && !is_left_child) {
                        branch_node.children = Children::BOTH; //TODO can be optimised when optimazation above has been made
                    }
                }).or_insert(TreeBranch { children: if is_left_child { Children::LEFT } else { Children::RIGHT }, descending: DataType::from(element_array_index) });
            }
            lss_branch.push(branch_map);
